...
struct _rb_ssl_ctx;
typedef struct _rb_ssl_ctx rb_ssl_ctx;
...
void rb_connect_tcp(rb_fde_t *, struct sockaddr *, struct sockaddr *, rb_socklen_t, CNCB *, void *, int);
void rb_connect_tcp_ssl(rb_fde_t *, struct sockaddr *, struct sockaddr *, rb_socklen_t, CNCB *, void *, int);
int rb_connect_sockaddr(rb_fde_t *, struct sockaddr *addr, rb_socklen_t len);
...
ssize_t rb_write(rb_fde_t *, const void *buf, size_t count);
ssize_t rb_writev(rb_fde_t *, struct rb_iovec *vector, int count);

ssize_t __rb_must_check rb_read(rb_fde_t *, void *buf, size_t count);
int __rb_must_check rb_pipe(rb_fde_t **, rb_fde_t **, const char *desc);

rb_ssl_ctx * __rb_must_check rb_setup_ssl_server(const char *cacert, const char *cert, const char *keyfile, const char *dhfile, const char *ssl_cipher_list, const char *named_curve, rb_tls_ver_t tls_min_ver);
rb_ssl_ctx * __rb_must_check rb_setup_ssl_client(const char *ssl_cipher_list, const char *cert, const char *keyfile);
int __rb_must_check  rb_ssl_listen(rb_fde_t *, int backlog, bool defer_accept);
int __rb_must_check rb_listen(rb_fde_t *, int backlog, bool defer_accept);

const char *rb_inet_ntop_sock(struct sockaddr *src, char *dst, rb_socklen_t size);
int rb_inet_pton_sock(const char *src, struct sockaddr *dst);
int rb_inet_pton(int af, const char *src, void *dst);
const char *rb_inet_ntop(int af, void *src, char *dst, rb_socklen_t len);
...
const char *rb_ssl_get_strerror(rb_fde_t *F);

rb_fde_t *rb_get_fde(int fd);

ssize_t rb_send_fd_buf(rb_fde_t *xF, rb_fde_t **F, unsigned int count, void *data, size_t datasize, pid_t pid);
ssize_t rb_recv_fd_buf(rb_fde_t *F, void *data, size_t datasize, rb_fde_t **xF, unsigned int count);
